...
// Sessions tagged before the rename from claudemux are still running, so
// their kind must survive being listed by the renamed tool.
func TestLegacyTaggedSessionsAreStillRecognised(t *testing.T) {
	requireTmux(t)

	name := testName(t, "legacy")
	if _, err := run("new-session", "-d", "-s", name, "sh"); err != nil {
		t.Fatalf("new-session: %v", err)
	}
	t.Cleanup(func() { _ = Kill(name) })

	// Tag it the way the old version would have.
	if _, err := run("set-option", "-t", optionTarget(name), legacyOptManaged, "1"); err != nil {
		t.Fatalf("set legacy managed option: %v", err)
	}
	if _, err := run("set-option", "-t", optionTarget(name), legacyOptKind, KindClaude); err != nil {
		t.Fatalf("set legacy kind option: %v", err)
	}
	...
	if !got.Managed {
		t.Error("a legacy-tagged session should still count as managed")
	}
	if got.Kind != KindClaude {
		t.Errorf("kind = %q, want %q", got.Kind, KindClaude)
	}
...
// The new options must win when both are somehow present.
func TestNewTagsTakePrecedenceOverLegacy(t *testing.T) {
	requireTmux(t)

	name := testName(t, "both")
	created, err := New(NewOptions{Name: name, Command: "sh", Kind: KindCodex})
	if err != nil {
		t.Fatalf("New: %v", err)
	}
	t.Cleanup(func() { _ = Kill(created) })

	if _, err := run("set-option", "-t", optionTarget(created), legacyOptKind, KindShell); err != nil {
		t.Fatalf("set legacy kind option: %v", err)
	}
	...
	if got.Kind != KindCodex {
		t.Errorf("kind = %q, want %q - the current tag should win", got.Kind, KindCodex)
	}
}
