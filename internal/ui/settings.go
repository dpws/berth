...
func settingsList() []setting {
	return []setting{
		...
		flag("notify_bell", "Bell", "Ring the terminal at the moments below",
			"rings", "silent",
			func(c config.Config) bool { return c.NotifyBell },
			func(c *config.Config, v bool) { c.NotifyBell = v }),
		flag("notify_desktop", "Desktop notification", "Ask the terminal to raise one, naming the session",
			"raised", "off",
			func(c config.Config) bool { return c.NotifyDesktop },
			func(c *config.Config, v bool) { c.NotifyDesktop = v }),
		flag("notify_waiting", "Tell me: waiting", "A session blocked on a question or a permission prompt",
			"yes", "no",
			func(c config.Config) bool { return c.NotifyWaiting },
			func(c *config.Config, v bool) { c.NotifyWaiting = v }),
		flag("notify_agent", "Notify via the agent", "Raise it on the machine running berth-clipd, which has words",
			"raised", "off",
			func(c config.Config) bool { return c.NotifyAgent },
			func(c *config.Config, v bool) { c.NotifyAgent = v }),
		flag("notify_taskbar", "Taskbar", "Mark the terminal in the taskbar while a session waits",
			"marked", "off",
			func(c config.Config) bool { return c.NotifyTaskbar },
			func(c *config.Config, v bool) { c.NotifyTaskbar = v }),
		flag("notify_idle", "Tell me: finished", "A turn that ended",
			"yes", "no",
			func(c config.Config) bool { return c.NotifyIdle },
			func(c *config.Config, v bool) { c.NotifyIdle = v }),
